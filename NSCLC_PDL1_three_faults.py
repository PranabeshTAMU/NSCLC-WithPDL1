import numpy as np
def NSCLC_PDL1_three_faults(fault1, fault2, fault3, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14):
#inputs 
    K_Ras = 0
    EGF = 0
    TGFalpha = 0
...
# Drugs
    ...
    Dabrafenib_Trametinib=x5#=1 #Targets RAF
    Durvalumab=x6#Targets PD_L1
    Alpelisib=x7#=1 #Targets PI3K
    Enzastaurin=x8#=1 #Targets PKC
    Lumakras=x9#=1 #Targets KRAS
    RG7388=x10#=1 #Targets MDM2
    Ribociclib=x11#=1 #Targets CDK4/6
    STAT_Inhibitor=x12#=1 #Targets STAT3/5
    Everolimus=x13#=1 #Targets mTOR
    Lorlatinib=x14#=1 #Targets EM4ALK
    ...
    if fault1 == 28 or fault2 == 28 or fault3 == 28:
        ...
        MEK = int(TRAF6 or (RAF and (not Dabrafenib_Trametinib))) 
    ...
    if fault1 == 31 or fault2 == 31 or fault3 == 31:
        ...
        CASP9 = int(not(PKB_Akt))        
        
    if fault1 == 32 or fault2 == 32 or fault3 == 32:         
        Forkhead = 0
        
    else:
        Forkhead = int((not(PKB_Akt)) or MST1)      
    
    if fault1 == 33 or fault2 == 33 or fault3 == 33:        
        IKK = 1
    else:
        IKK = int(TRIF or PKB_Akt)
    ...
    if fault1 == 36 or fault2 == 36 or fault3 == 36:        
        ...
        NFKB = int(PKB_Akt or IKK)                  

    if fault1 == 37 or fault2 == 37 or fault3 == 37:        
        PD_L1 = 1
    else:
        PD_L1 = int(AP1 or NFAT or (STAT3by5 and (not STAT_Inhibitor)) or NFKB or DNA_Damage)                  
    ...
    if fault1 == 46 or fault2 == 46 or fault3 == 46:
        ...
        CyclinD1 = int((not RASSF1) or ERK or (STAT3by5 and (not STAT_Inhibitor)) or (not P16))
    ...
    Uncontrolled_Proliferation =  int(CyclinD1 or S6K or RARbeta or Rb)
    Apoptosis =  int((not RARbeta) and P21 and FHIT and BAD and CASP9 and Forkhead)
    T_Cell = int(not(PD_L1 and (not Durvalumab)))


    output1 = [Uncontrolled_Proliferation,Apoptosis, T_Cell]
    #print(output1)
    output2 = [0, 1, 1]  # Ideal Output
